...
impl Write for UnixConnection {
    ...
    fn flush(&mut self) -> Result<(), CPError> {
        Ok(self.0.flush().map_err(|_| CPError::FlushError)?)
    }
}
...
fn main() {
    ...
    loop {
        ...
        let http_req = client.post(url.clone() + &req.endpoint).json(&req.body);
        debug!("HTPP request - {:#?}", http_req);
        ...
        proxy.write_json(&json!(resp)).unwrap();
    }
